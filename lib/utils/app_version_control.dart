...
const String gitHubUser = "Al3x18";
const String repoName = "apps_version_control";
const String jsonFileName = "reports_min_required_app_version.json";
...
class AppVersionControl {
  ...
  Future<String> get latestVersionAvailable async {
    ...
    final token = getEnv('GITHUB_TOKEN');
    if (token.isEmpty) return latestVersionAvailable;
    final url = Uri.parse(urlGitHubRepo);
    final response = await http
        .get(url, headers: {'Authorization': 'token $token'});
    final data = jsonDecode(response.body);
